...
export function useDerivedZapInfo(
    ...
    // pair
    const [pairState, pair] = useV2Pair(network, currency0 ?? undefined, currency1 ?? undefined)
    ...
    const currencyZeroTrade = useV2TradeExactIn(network, tradeAmount, currency0 ?? undefined)
    const currencyOneTrade = useV2TradeExactIn(network, tradeAmount, currency1 ?? undefined)
