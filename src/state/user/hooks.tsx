 ...
 */
export function toV2LiquidityToken({ tokens, network} : { tokens: Token[], network: ChainId }): Token {
    return tokens[0] && tokens[1] && tokens[0].chainId === tokens[1].chainId && FACTORY_ADDRESS[tokens[0].chainId]
        ? new Token(tokens[0].chainId, computePairAddress({ network: network, tokenA: tokens[0], tokenB: tokens[1] }), 18, "ETA-V2", "Eta Swap V2")
        : undefined
}
