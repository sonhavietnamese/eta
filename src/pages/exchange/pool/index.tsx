...
export default function Pool() {
    ...
    const tokenPairsWithLiquidityTokens = useMemo(() => {
        ...
        return trackedTokenPairs.map((tokens) => ({
            liquidityToken: toV2LiquidityToken({ tokens, network }),
            tokens,
        }))
    }, [chainId])
    ...
    return (
        <>
            <Head>
                <title>{`Pools`} | Eta</title>
                <meta
                    key="description"
                    name="description"
                    content="Eta liquidity pools are markets for trades between the two tokens, you can provide these tokens and become a liquidity provider 
