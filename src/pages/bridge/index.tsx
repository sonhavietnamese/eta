...
import { BridgeContextName, chainIdToString, DEPLOYMENTS, tokenAddressToIbToken, tokenNameToAddress } from "../../constants"
...
export default function Bridge() {
    ...
    const { account: bridgeAccount, chainId: bridgeChainId, library, activate } = useWeb3React(BridgeContextName)

    const availableChains = [ChainId.MANTLE_TESTNET, ChainId.MATIC_TESTNET, ChainId.CHIADO, ChainId.POLYGON_ZKEVM, ChainId.SCROLL]
    ...
    const currentChainFrom = 
        SUPPORTED_NETWORKS[network] && { id: network, icon: NETWORK_ICON[network], name: NETWORK_LABEL[network] }
    ...
    useEffect(() => {
        ...
        if (bridgeChainId) {
            if (bridgeChainId == chainTo.id) {
                setChainTo(chainFrom)
            }
            setChainFrom({ id: bridgeChainId, icon: NETWORK_ICON[bridgeChainId], name: NETWORK_LABEL[bridgeChainId] })
        }
    }, [activate, bridgeChainId, activeAccount, activeChainId])
    ...
    const [chainTo, setChainTo] = useState<Chain | null>(
        bridgeChainId == ChainId.MOONRIVER ? DEFAULT_CHAIN_FROM : DEFAULT_CHAIN_TO
    )
    ...
    const liquidityToken = new Token(activeChainId, DEPLOYMENTS[chainIdToString(network)].tokens.DCT, 18, "DCT", "DCT")
    const typedDepositValue = tryParseAmount(currencyAmount, liquidityToken)
    const [approvalState, approve] = useApproveCallback(typedDepositValue, DEPLOYMENTS[chainIdToString(network)].bridge.Bridge)
    ...
    useEffect(() => {
        let weth = "wETH"
        if(chainFrom.id === ChainId.MATIC_TESTNET) {
            weth = "wMATIC"
        } else if(chainFrom.id === ChainId.CHIADO) {
            weth = "xDAI"
        }
        const meTokens = [weth, "DCT", "MAV", "WLR", "DIB"]
    ...
    const handleChainFrom = useCallback(
        (chain: Chain) => {
            ...
            if (changeTo.id !== ChainId.CHIADO && chain.id !== ChainId.CHIADO) {
    ...
    const handleChainTo = useCallback(
        (chain: Chain) => {
            ...
            if (changeFrom.id !== ChainId.CHIADO && chain.id !== ChainId.CHIADO) {
    ...
    const handleCurrencySelect = useCallback(
        (currency: Currency) => {
            ...
            console.debug("Currency select:", currency)
            handleTypeInput("")
    ...
    const buttonDisabled =
        (chainFrom && chainFrom.id !== activeChainId) ||
        !currency0 ||
        !currencyAmount ||
        currencyAmount == "" ||
        !aboveMin() ||
                        ...
                        <div className="p-4 text-center">
                            <div className="justify-between space-x-3 items-center">
                                <Typography component="h3" variant="base">
                                    {`Lightning-quick, cross-chain token swaps`}
                                </Typography>
                            </div>
                        </div>
