...
import ETA_DISTRIBUTOR_ABI from "../constants/abis/solar-distributor.json"
...
import EtaFactoryABI from "../constants/artifacts/contracts/swap/EtaFactory.sol/EtaFactory.json"
import EtaRouter02ABI from "../constants/artifacts/contracts/swap/EtaRouter02.sol/EtaRouter02.json"
...
export function useEtaDistributorContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && ETA_DISTRIBUTOR_ADDRESS[chainId], EtaDistributor.abi, withSignerIfPossible)
}

export function useEtaVaultContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && ETA_VAULT_ADDRESS[chainId], SOLAR_VAULT_ABI, withSignerIfPossible)
}
...
export function useSwapFactoryContract(withSignerIfPossible?: boolean): Contract | null {
    const { chainId } = useActiveWeb3React()
    return useContract(chainId && FACTORY_ADDRESS[chainId], EtaFactoryABI.abi, withSignerIfPossible)
}
...
export function useRouterContract(useArcher = false, withSignerIfPossible?: boolean): Contract | null {
    ...
    const abi = useArcher ? ARCHER_ROUTER_ABI : EtaRouter02ABI.abi
