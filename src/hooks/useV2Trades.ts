import { ChainId, Currency, CurrencyAmount, Pair, Trade, TradeType } from "../sdk"
...
function useAllCommonPairs(network: ChainId, currencyA?: Currency, currencyB?: Currency): Pair[] {
    const allCurrencyCombinations = useAllCurrencyCombinations(currencyA, currencyB)
    const allPairs = useV2Pairs(allCurrencyCombinations, network)
...
export function useV2TradeExactIn(
    ...
    const allowedPairs = useAllCommonPairs(network, currencyAmountIn?.currency, currencyOut)

    return useMemo(() => {
        if (currencyAmountIn && currencyOut && allowedPairs.length > 0) {
            if (maxHops === 1) {
                return (
                    Trade.bestTradeExactIn(network, allowedPairs, currencyAmountIn, currencyOut, {
                        maxHops: 1,
                        maxNumResults: 1,
                    })[0] ?? null
                )
...
export function useV2TradeExactOut(
    ...
    const allowedPairs = useAllCommonPairs(network, currencyIn, currencyAmountOut?.currency)

    return useMemo(() => {
        if (currencyIn && currencyAmountOut && allowedPairs.length > 0) {
            if (maxHops === 1) {
                return (
                    Trade.bestTradeExactOut(network, allowedPairs, currencyIn, currencyAmountOut, {
                        maxHops: 1,
                        maxNumResults: 1,
                    })[0] ?? null
                )
