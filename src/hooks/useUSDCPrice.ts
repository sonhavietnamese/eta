 ...
 */
export default function useUSDCPrice(network: ChainId, currency?: Currency): Price<Currency, Token> | undefined {
    const { chainId } = useActiveWeb3React()

    const amountOut = chainId ? STABLECOIN_AMOUNT_OUT[chainId] : undefined
    const stablecoin = amountOut?.currency

    const v2USDCTrade = useV2TradeExactOut(network, currency, amountOut, {
        maxHops: 3,
    })
...
export function useUSDCValue(network: ChainId, currencyAmount: CurrencyAmount<Currency> | undefined | null) {
    const price = useUSDCPrice(network, currencyAmount?.currency)
