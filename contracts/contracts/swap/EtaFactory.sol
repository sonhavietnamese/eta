...
contract EtaFactory is IEtaFactory {
    bytes public constant creationCode = type(EtaPair).creationCode; 
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(EtaPair).creationCode));
        ...
        returns(address pair)
    {
        ...
        EtaPair(pair).initialize(token0, token1);
