...
import "./IEtaERC20.sol";

 
contract EtaDistributor is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _poolId, uint256 _amount) public nonReentrant {
        // require(
        //     block.number >= startBlock,
        //     "EtaDistributor: Can not deposit before start"
        // );
    ...
    function safeTauTransfer(address _to, uint256 _amount) internal {
        if(tau.balanceOf(address(this)) > totalTauInPools) {
            //TauBal = total Tau in EtaDistributor - total Tau in Tau pools, this will make sure that EtaDistributor never transfer rewards from deposited Tau pools
